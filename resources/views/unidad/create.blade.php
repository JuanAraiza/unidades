...
<div class="row col-md-12 mb-2">
 ...
 <img id="imgPreview" style="object-fit: cover;width: 100%; height:100%;" class="img-fluid object-fit-cover border rounded" src="https://thumb.ac-illust.com/b1/b170870007dfa419295d949814474ab2_w.jpeg" alt="">
...
<x-adminlte-select2 name="tunidad"  data-placeholder="Selecciona Tipo....">
        <option @selected(old('tunidad') == 'Vehiculos') value="Vehiculos">Vehículos</option>
        <option @selected(old('tunidad') == 'Maquinaria') value="Maquinaria">Maquinaria</option>
        <option @selected(old('tunidad') == 'Herramientas') value="Herramientas">Herramientas</option>
        <option @selected(old('tunidad') == 'Otros') value="Otros">Otros</option>
</x-adminlte-select2>
</div>
</div>
...
<x-adminlte-select2 name="combustible"  data-placeholder="Selecciona Tipo....">
        <option @selected(old('combustible') == 'Gasolina') value="Gasolina">Gasolina</option>
        <option @selected(old('combustible') == 'Diesel') value="Diesel">Diesel</option>
        <option @selected(old('combustible') == 'Gas LP') value="Gas LP">Gas LP</option>
</x-adminlte-select2>
</div>
</div>
@php
...
<x-adminlte-select2 name="tipov"  data-placeholder="Selecciona Tipo Vehiculo....">
    @foreach($tipos as $tipov)
        <option @selected(old('tipov') == $tipov->id) value="{{ $tipov->id }}">{{ $tipov->tipo }}</option>
    @endforeach
...
<x-adminlte-select2 name="estatus"  data-placeholder="Selecciona Estatus....">
        <option @selected(old('estatus') == '1') value="1">Disponible</option>
        <option @selected(old('estatus') == '2') value="2">En Taller</option>
        <option @selected(old('estatus') == '3') value="3">Fuera de Servicio</option>
</x-adminlte-select2>
</div>
</div>
...
<x-adminlte-select2 name="inicio_est"  data-placeholder="Selecciona Inicio....">
        <option @selected(old('inicio_est') == '1') value="1">Fecha de Registro</option>
        <option @selected(old('inicio_est') == '2') value="2">Fecha de Compra</option>
</x-adminlte-select2>
</div>
</div>
...
<x-adminlte-select2 name="medida_usu"  data-placeholder="Selecciona Inicio....">
        <option @selected(old('medida_uso') == 'Kilometros') value="Kilometros">Kilometros</option>
        <option @selected(old('medida_uso') == 'Horas') value="Horas">Horas</option>
</x-adminlte-select2>
</div>
</div>
...
<x-adminlte-select2 name="medida_con"  data-placeholder="Selecciona Inicio....">
        <option @selected(old('medida_con') == 'Litros') value="Litros">Litros</option>
</x-adminlte-select2>
</div>
</div>
...
<x-adminlte-select2 name="area_id"  id="area_id" data-placeholder="Selecciona Area....">
  ...
  <div class="col-md-3"> 
        <div class="form-group">
            ...
            <input type="text" name="cilindros" value="{{ old('cilindros') }}" class="form-control"
     ...
     <div class="col-md-3"> 
        <div class="form-group">
            <label>Uso</label>
            <input type="text" name="uso" value="{{ old('uso') }}" class="form-control"
               placeholder="Uso"  >
        </div>
        @error('uso')
            <span style="color:crimson;">
    ...
    <div class="col-md-12"> 
        <div class="form-group">
            ...
            <textarea name="detalles" rows="5" class="form-control"
               placeholder="Detalles"  >{{ old('detalles') }}</textarea>
        </div>
        @error('detalles')
            <span style="color:crimson;">
                {{$message}}
...
@section('js')
    ...
    $(document).ready(function() {
           /* var dependenciaSeleccionada = $('#dependencia').val();
            alert(dependenciaSeleccionada);
            // Carga las subcategorías según la categoría seleccionada
            cargarAreas(dependenciaSeleccionada);
*/
            // Establece un evento listener para el cambio del select de categorías
            /*
